...
describe("C2NSale", function () {
  ...
  const DEPLOYER_PRIVATE_KEY = process.env.CHAITEST_PRIVATE_KEY;
  ...
  async function participate(params) {
    ...
    const sig = signParticipation(
      userAddress,
      participationAmount,
      await C2NSale.getAddress(),
      DEPLOYER_PRIVATE_KEY
    ); // backend signature
    return await C2NSale.connect(registrant).participate(
      sig,
      participationAmount,
      {
        value: value,
  ...
  async function setSaleParams(params) {
    ...
    const token = firstOrDefault(params, "token", await C2NToken.getAddress());
    ...
    return await C2NSale.setSaleParams(
      token,
      saleOwner,
      tokenPriceInETH,
      amountOfTokensToSell,
  ...
  async function setRegistrationTime(params) {
    ...
    return C2NSale.setRegistrationTime(
  ...
  async function setVestingParams() {
    ...
    await C2NSale.setVestingParams(
  ...
  beforeEach(async function () {
    ...
    C2NToken = await BreTokenFactory.deploy(
      "C2N",
      "C2N",
    ...
    await AllocationStaking.initialize(
      await C2NToken.getAddress(),
      REWARDS_PER_SECOND,
      startTimestamp,
      await SalesFactory.getAddress()
    );

    await AllocationStaking.add(1, await C2NToken.getAddress(), false);
  ...
  context("setup", async function () {
    ...
    describe("Set sale parameters", async function () {
      it("Should set the sale parameters", async function () {
        ...
        const blockTimestamp = (await ethers.provider.getBlock("latest"))
          .timestamp;
        const token = await C2NToken.getAddress();
    ...
    describe("Set sale registration times", async function () {
      it("Should set the registration times", async function () {
        ...
        // When
        await C2NSale.setRegistrationTime(
          registrationTimeStarts,
          registrationTimeEnds
        );
    ...
    describe("Set sale startTime", async function () {
      ...
      it("Should set sale token", async function () {
        ...
        // When
        await C2NSale.setSaleToken(await BreToken2.getAddress());
  ...
  context("update", async function () {
    describe("Update token price", function () {
      ...
      it("Should not allow non-admin to set token price", async function () {
        ...
        // Then
        await expect(C2NSale.updateTokenPriceInETH(price)).to.be.revertedWith(
          "Only admin can call this function."
        );
      });
    ...
    describe("Postpone sale", async function () {
      it("Should postpone the sale", async function () {
        ...
        // Then
        expect((await C2NSale.sale()).saleStart).to.equal(
      ...
      it("Should not allow non-admin to postpone sale", async function () {
        ...
        // Then
        await expect(C2NSale.postponeSale(timeToShift)).to.be.revertedWith(
          "Only admin can call this function."
        );
      });
    ...
    describe("Extend registration period", async function () {
      it("Should extend the registration period", async function () {
        ...
        const currentRegistrationEnd = parseInt(
          (await C2NSale.registration()).registrationTimeEnds
        );
        ...
        // Then
        expect((await C2NSale.registration()).registrationTimeEnds).to.equal(
      ...
      it("Should not allow non-admin to extend registration period", async function () {
        ...
        // Then
        await expect(
          C2NSale.extendRegistrationPeriod(timeToAdd)
    ...
    describe("Set max participation", async function () {
      ...
      it("Should emit MaxParticipationSet", async function () {
        ...
        // Then
        await expect(C2NSale.setCap(cap)).to.emit(
          C2NSale,
          "MaxParticipationSet"
        );
      });
      ...
      it("Should not allow non-admin to set max participation", async function () {
        ...
        // Then
        await expect(C2NSale.setCap(cap)).to.be.revertedWith(
          "Only admin can call this function."
        );
      });
    ...
    describe("Deposit tokens", async function () {
      it("Should allow sale owner to deposit tokens", async function () {
        ...
        // Then
        const balance = await C2NToken.balanceOf(await C2NSale.getAddress());
      ...
      it("Should not allow non-sale owner to deposit tokens", async function () {
        ...
        // Then
        await expect(C2NSale.depositTokens()).to.be.revertedWith(
  ...
  context("Registration", async function () {
    describe("Register for sale", async function () {
      ...
      it("Should not register after registration ends", async function () {
        ...
        // Then
        await expect(C2NSale.registerForSale(sig, 0)).to.be.revertedWith(
      ...
      it("Should emit UserRegistered event", async function () {
        ...
        await expect(C2NSale.registerForSale(sig, 0))
          .to.emit(C2NSale, "UserRegistered")
    ...
    xdescribe("Get registration info", async function () {
      ...
      it("Should return initial registration info", async function () {
        ...
        // Then
        const registrationTimeEnds = (await C2NSale.registration())
          .registrationTimeEnds;
  ...
  context("Signature validation", async function () {
    describe("Check registration signature", async function () {
      it("Should succeed for valid signature", async function () {
        ...
        // Then
        expect(
          await C2NSale.checkRegistrationSignature(sig, deployer.address)
        ).to.be.true;
      });
      ...
      it("Should revert if signature has wrong length", async function () {
        ...
        await expect(
          C2NSale.checkRegistrationSignature(sig.subarray(1), deployer.address)
        ).to.revertedWithCustomError(C2NSale, "ECDSAInvalidSignatureLength");
      ...
      it("Should revert if signature has wrong format", async function () {
        ...
        // Then
        // await expect(
        //   C2NSale.checkRegistrationSignature(sig, deployer.address)
      ...
      it("Should fail if signature is applied to hash instead of prefixed EthereumSignedMessage hash", async function () {
        ...
        const digest = ethers.keccak256(
          ethers.solidityPacked(
            ["address", "address"],
            [deployer.address, await C2NSale.getAddress()]
          )
        );
        const { v, r, s } = ethUtil.ecsign(
    ...
    describe("Check participation signature", async function () {
      it("Should succeed for valid signature", async function () {
        ...
        // Then
        expect(
          await C2NSale.checkParticipationSignature(sig, deployer.address, 100)
        ).to.be.true;
      });
      ...
      it("Should revert if signature has wrong length", async function () {
        ...
        // Then
        await expect(
          C2NSale.checkParticipationSignature(
            sig.subarray(1),
            deployer.address,
            100
          )
      ...
      it("Should revert if signature has wrong format", async function () {
        ...
        await expect(
          C2NSale.checkParticipationSignature(sig, deployer.address, 100)
        ).to.revertedWithCustomError(C2NSale, "ECDSAInvalidSignature");
      });
  ...
  context("Participation", async function () {
    describe("Participate", async function () {
      ...
      it("Should allow multiple users to participate", async function () {
        ...
        // Then
        const sale = await C2NSale.sale();
        const isParticipatedDeployer = await C2NSale.isParticipated(
          deployer.address
        );
        const isParticipatedAlice = await C2NSale.isParticipated(alice.address);
        const participationDeployer = await C2NSale.userToParticipation(
          deployer.address
        );
        const participationAlice = await C2NSale.userToParticipation(
          alice.address
        );
      ...
      it("Should not participate with invalid signature", async function () {
        ...
        // Then
        await expect(
          C2NSale.participate(sig, PARTICIPATION_AMOUNT, {
    ...
    describe("Withdraw tokens", async function () {
      it("Should withdraw user's tokens", async function () {
        ...
        await C2NToken.transfer(
          await C2NSale.getAddress(),
          "10000000000000000000"
        );
        const previousBalance = BigInt(
          await C2NToken.balanceOf(deployer.address)
        );
      ...
      it("Should not withdraw twice", async function () {
        ...
        // Then
        await expect(C2NSale.withdrawTokens(0)).to.be.revertedWith(
      ...
      it("Should emit TokensWithdrawn event", async function () {
        ...
        // Then
        await expect(C2NSale.withdrawTokens(0))
          .to.emit(C2NSale, "TokensWithdrawn")
          .withArgs(
            deployer.address,
    ...
    describe("Withdraw earnings and leftover", async function () {
      xit("Should withdraw sale owner's earnings and leftovers", async function () {
        ...
        const sale = await C2NSale.sale();
        console.log(
          parseInt(sale.amountOfTokensToSell),
          parseInt(sale.totalTokensSold)
        );
        ...
        const contractTokenBalance = await C2NToken.balanceOf(
          await C2NSale.getAddress()
        );

        expect(currentBalance).to.equal(
          previousBalance.add(PARTICIPATION_VALUE)
        );
        expect(currentTokenBalance).to.equal(
          previousTokenBalance.add(
            AMOUNT_OF_TOKENS_TO_SELL - PARTICIPATION_VALUE / TOKEN_PRICE_IN_ETH
          )
        );
        expect(contractBalance).to.equal(0);
        expect(contractTokenBalance).to.equal(
          PARTICIPATION_VALUE / TOKEN_PRICE_IN_ETH
        );
      });
      ...
      it("Should not allow non-sale owner to withdraw", async function () {
        ...
        // Then
        await expect(
          C2NSale.connect(bob).withdrawEarningsAndLeftover()
      ...
      xit("Should burn leftover if requested", async function () {
        ...
        await ethers.provider.send("evm_increaseTime", [
          ROUNDS_START_DELTAS[0] - REGISTRATION_TIME_STARTS_DELTA,
        ]);
        await ethers.provider.send("evm_mine");
        ...
        await ethers.provider.send("evm_increaseTime", [
          SALE_END_DELTA - ROUNDS_START_DELTAS[0],
        ]);
        await ethers.provider.send("evm_mine");
        ...
        expect(burnedTokenBalance).to.equal(
          AMOUNT_OF_TOKENS_TO_SELL - PARTICIPATION_VALUE / TOKEN_PRICE_IN_ETH
        );
      });
      ...
      xit("Should not crash if leftover is 0", async function () {
        // Given
        await runFullSetup({
          amountOfTokensToSell: Math.floor(
            (PARTICIPATION_VALUE / TOKEN_PRICE_IN_ETH) * MULTIPLIER
          ),
        });
      ...
      xit("Should not crash if earnings are 0", async function () {
        ...
        expect(currentTokenBalance).to.equal(
          previousTokenBalance.add(AMOUNT_OF_TOKENS_TO_SELL)
        );
        expect(contractBalance).to.equal(0);
