...
function getSavedContractAddresses() {
  ...
  try {
    json = fs.readFileSync(
      path.join(__dirname, "../deployments/contract-addresses.json")
    );
  } catch (err) {
    json = "{}";
  }
...
function getSavedContractABI() {
  ...
  try {
    json = fs.readFileSync(
      path.join(__dirname, `../deployments/contract-abis.json`)
    );
  } catch (err) {
    json = "{}";
  }
...
function getSavedProxyABI() {
  ...
  try {
    json = fs.readFileSync(
      path.join(__dirname, `../deployments/proxy-abis.json`)
    );
  } catch (err) {
    json = "{}";
  }
...
function getSalesConfig(network) {}

function refreshSalesTime(network) {}
module.exports = {
