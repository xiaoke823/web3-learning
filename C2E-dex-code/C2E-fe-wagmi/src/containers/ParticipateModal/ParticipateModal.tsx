...
import { parseUnits, toBigInt } from '@src/lib/viem';
...
export default function ParticipateModal(props) {
  ...
  const calEth = useCallback((breValue) => {
    let eth;
    try {
      const scaledValue = BigInt(breValue || 0) * 10n ** BigInt(18 - decimals);
      eth = formatEther(scaledValue * toBigInt(data.tokenPriceInPT || 0), 8);
    } catch (e) {
      eth = 0;
    }
