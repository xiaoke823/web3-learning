...
import { formatUnits as formatUnitsValue, toBigInt } from "@src/lib/viem";
...
export default function Pool() {
  ...
  const amountBought = useMemo(() => {
    return (participateInfo && toBigInt(participateInfo[0])) || 0n;
  }, [participateInfo]);

  const amountETHPaid = useMemo(() => {
    return (participateInfo && toBigInt(participateInfo[1])) || 0n;
  }, [participateInfo]);

  const timeParticipated: number = useMemo(() => {
    return Number((participateInfo && parseWei(participateInfo[2])) || 0);
  }, [participateInfo]);
  ...
  useEffect(() => {
    const bigNumSec2Milsec = (bigSec) => {
      return (bigSec && toBigInt(bigSec).toString()) || "";
    };
    if (projectInfo) {
      // init sale contract
  ...
  async function participate(value) {
    ...
    const paymentAmount =
      (toBigInt(projectInfo.tokenPriceInPT) * BigInt(Number(value.value || 0))) /
      10n ** BigInt(18 - Number(decimals));
    /**
     * 购买代币
    ...
    return approve(
      saleAddress,
      Number(formatUnitsValue(paymentAmount, depositDecimals)),
      depositDecimals
    )
        ...
        console.log(
          {
            ...
            options,
            bg: BigInt(allocationTop + ""),
          },
          "params---------"
        );

        return saleContract
          .participate(signBuffer, BigInt(allocationTop + ""), options)
          .then((transaction) => {
            return transaction.wait();
          })
          .then((transaction) => {
