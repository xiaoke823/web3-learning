...
export const useStake = () => {
  ...
  async function approve(contractAddress, amount, decimals = 18) {
    ...
    console.log({ depositTokenAddress, contractAddress, amount: ethers.utils.parseUnits(approveAmount + '', decimals) }, depositTokenContract)
    return (
      (depositTokenContract.approve &&
        depositTokenContract.approve(contractAddress, etherUtils.parseUnits('10000', 18)))
        .then(transaction => {
